...
use hifive1::{pin, sprint, sprintln};
use riscv_rt::entry;
...
fn main() -> ! {
    ...
    sprintln!("Set mode to active.");
    wifi.send("AT+CWMODE=1\r\n");
    Delay.delay_ms(20u32);
    sprintln!("resp: {:?}", wifi.recv(&mut buffer));

    sprintln!("Check status.");
    wifi.send("AT+CWJAP?\r\n");
    Delay.delay_ms(20u32);
    sprintln!("resp: {:?}", wifi.recv(&mut buffer));

    sprintln!("Start connection.");
    wifi.send("AT+CWJAP=\"Pug's Life\",\"tak Urcite21\"\r\n");
    loop {
        Delay.delay_ms(200u32);
        let rcv = wifi.recv(&mut buffer);
        if let Ok(result) = rcv {
            sprintln!("resp: {:?}", result);
        }
        sprint!(".");
    }
